...
// Used only for START_PLAY_ROUND initial deal — auto-resolves Flip Three against the drawer.
function dealCardsInitial(deck, hand, playerId, dealLog, count) {
  let currentDeck = deck;
  let currentHand = hand;
  let currentLog = dealLog;
  let anyReshuffle = false;

  for (let i = 0; i < count; i++) {
    if (currentHand.status === "busted") break;

    const result = dealOneCard(currentDeck, currentHand, playerId, currentLog);
    currentDeck = result.deck;
    currentHand = result.hand;
    currentLog = result.dealLog;
    if (result.reshuffled) anyReshuffle = true;

    if (result.card.type === "action" && result.card.value === "Freeze") break;

    // During initial deal, Flip Three auto-targets the drawer (deal 2 more to self)
    if (result.pendingType === "flip_three_target" && currentHand.status !== "busted") {
      const extra = dealCardsInitial(currentDeck, currentHand, playerId, currentLog, 2);
      currentDeck = extra.deck;
      currentHand = extra.hand;
      currentLog = extra.dealLog;
      if (extra.reshuffled) anyReshuffle = true;
    }

    // During initial deal, duplicate Second Chance is just discarded (no one else to give it to yet)
  }

  return { deck: currentDeck, hand: currentHand, dealLog: currentLog, reshuffled: anyReshuffle };
}

// Deals count cards to a target player during Flip Three resolution.
// Collects queued Flip Threes / Second Chance gifts for later resolution.
function dealFlipThreeCards(deck, playerHands, targetPlayerId, dealLog, count) {
  let currentDeck = deck;
  ...
  for (let i = 0; i < count; i++) {
    if (currentHand.status !== "playing") break;

    const result = dealOneCard(currentDeck, currentHand, targetPlayerId, currentLog);
    currentDeck = result.deck;
    currentHand = result.hand;
    currentLog = result.dealLog;
    if (result.reshuffled) anyReshuffle = true;
    ...
    // Flip Three drawn during resolution: queue it (finish current 3 first)
    if (result.pendingType === "flip_three_target") {
      flipThreeQueue.push({ type: "flip_three_target", chooserId: targetPlayerId });
    }

    // Duplicate Second Chance during resolution: queue for gifting
  ...
  return { deck: currentDeck, playerHands: currentHands, dealLog: currentLog, reshuffled: anyReshuffle, flipThreeQueue };
}
